...
<body class="bg-gray-50" x-data="{
    ...
    // Methods
    addNewPelanggan() {
        this.currentPelanggan = { id: null, kode: '', nama: '', alamat: '', email: '', point: 0, level: '' };
        this.showAddPelangganModal = true;
    },
    ...
    prepareDelete(pelanggan) {
        this.currentPelanggan = pelanggan;
        this.showDeleteConfirmModal = true;
    },

    saveNewPelanggan() {
        this.showAddPelangganModal = false;
    },
        ...
        <div class="flex-1 flex flex-col overflow-hidden">
            ...
            <main class="flex-1 overflow-y-auto bg-gray-50 p-6 custom-scrollbar content-area">
                ...
                <div class="space-y-6">
                    ...
                    <div class="bg-white rounded-xl shadow-sm overflow-hidden">
                        <div class="p-6 border-b border-gray-100">
                            <div class="flex items-center justify-between">
                                <h3 class="font-semibold text-gray-800">Daftar Pelanggan</h3>
                                <span class="text-sm text-gray-500">Total :
                                    {{ $totalPelanggan }} Pelanggan</span>
                            </div>
                        </div>
                                ...
                                <tbody class="divide-y divide-gray-100">
                                    @foreach ($pelanggans as $data)
                                        <tr class="hover:bg-gray-50 transition duration-150">
                                            <td class="py-4 px-6">{{ $data->kode }}</td>
                                            <td class="py-4 px-6">{{ $data->nama }}</td>
                                            <td class="py-4 px-6">{{ $data->alamat }}</td>
                                            <td class="py-4 px-6">{{ $data->email }}</td>
                                            <td class="py-4 px-6">{{ $data->point }}</td>
                                            <td class="py-4 px-6">{{ $data->level }}</td>
                                            <td class="py-4 px-6">
                                                <div class="flex space-x-2">
                                                    <button @click="editPelanggan({{ json_encode($data) }})"
                        ...
                        <!-- Empty Table State -->
                        <div class="p-8 text-center" x-show="filteredPelanggan().length === 0">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-12 w-12 mx-auto text-gray-400"
                                fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" />
                            </svg>
                            <p class="mt-2 text-sm text-gray-500">Tidak ada Pelanggan yang sesuai dengan filter Anda.
                            </p>
                    ...
                    <!-- Pagination -->
                    <div class="flex items-center justify-between">
                        <p class="text-sm text-gray-500">Menampilkan 1 - {{ $totalPelanggan }} dari
                            {{ $totalPelanggan }}
                            Pelanggan</p>
            ...
            <div x-show="showAddPelangganModal"
                ...
                <div @click.away="showAddPelangganModal = false"
                    ...
                    <div class="flex justify-between items-center mb-6">
                        ...
                        <button @click="showAddPelangganModal = false" class="text-gray-500 hover:text-gray-700">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none"
                                viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <form action="{{ route('pelanggan.store') }}" method="POST">
                        ...
                        <div class="space-y-4">
                            <!-- Kode Field -->
                            <div>
                                <label for="kode" class="block text-sm font-medium text-gray-700 mb-1">Kode
                                    Pelanggan</label>
                            ...
                            <div>
                                <label for="point"
                                    class="block text-sm font-medium text-gray-700 mb-1">Point</label>
                                <input type="number" id="point" name="point"
                                    class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                                    placeholder="Masukkan point" value="0" required>
                            </div>
                            ...
                            <div class="flex space-x-3 mt-6">
                                <button type="button" @click="showAddPelangganModal = false"
                                    class="flex-1 px-4 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition duration-200">
                                    Batal
                                </button>
                                <button type="submit"
            ...
            <div x-show="showEditPelangganModal"
                ...
                x-transition:enter-end="opacity-100" x-transition:leave="transition ease-in duration-200"
                x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0">
                <div @click.away="showEditPelangganModal = false"
                    class="bg-white rounded-xl shadow-xl max-w-md w-full p-6 mx-4"
                    x-transition:enter="transition ease-out duration-300"
                    x-transition:enter-start="transform scale-95 opacity-0"
                    x-transition:enter-end="transform scale-100 opacity-100"
                    ...
                    <div class="flex justify-between items-center mb-6">
                        ...
                        <button @click="showEditPelangganModal = false" class="text-gray-500 hover:text-gray-700">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none"
                                viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M6 18L18 6M6 6l12 12" />
                    ...
                    <form :action="`/pelanggan/${currentPelanggan.id}`" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="space-y-4">
                            <!-- Kode Field -->
                            <div>
                                <label for="edit-kode" class="block text-sm font-medium text-gray-700 mb-1">Kode
                                    Pelanggan</label>
                                <input type="text" id="edit-kode" name="kode" x-model="currentPelanggan.kode"
                                    class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                                    placeholder="Masukkan kode pelanggan" required>
                            </div>
                            ...
                            <div>
                                <label for="edit-nama" class="block text-sm font-medium text-gray-700 mb-1">Nama
                                    Pelanggan</label>
                                <input type="text" id="edit-nama" name="nama" x-model="currentPelanggan.nama"
                                    class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                                    placeholder="Masukkan nama pelanggan" required>
                            </div>
                            ...
                            <div>
                                <label for="edit-alamat"
                                    class="block text-sm font-medium text-gray-700 mb-1">Alamat</label>
                                <textarea id="edit-alamat" name="alamat" x-model="currentPelanggan.alamat"
                                    class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                                    placeholder="Masukkan alamat" required></textarea>
                            </div>
                            ...
                            <div>
                                ...
                                <input type="email" id="edit-email" name="email"
                                    x-model="currentPelanggan.email"
                                    class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                                    placeholder="Masukkan email" required>
                            </div>
                            ...
                            <!-- Point Field -->
                            <div>
                                <label for="edit-point"
                                    class="block text-sm font-medium text-gray-700 mb-1">Point</label>
                                <input type="number" id="edit-point" name="point"
                                    x-model="currentPelanggan.point"
                                    class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                            ...
                            <div>
                                <label for="edit-level"
                                    class="block text-sm font-medium text-gray-700 mb-1">Level</label>
                                <select id="edit-level" name="level" x-model="currentPelanggan.level"
                                    class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                                    required>
                                    <option value="">Pilih Level</option>
                                    <option value="Bronze">Bronze</option>
                            ...
                            <div class="flex space-x-3 mt-6">
                                <button type="button" @click="showEditPelangganModal = false"
                                    class="flex-1 px-4 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition duration-200">
                                    Batal
                                </button>
                                <button type="submit"
    ...
    <div class="fixed bottom-6 left-6 z-30" x-data="{ showQuickActions: false }">
        <div x-show="showQuickActions" @click.away="showQuickActions = false"
            ...
            <div class="space-y-2">
                <button @click="addNewPelanggan(); showQuickActions = false"
